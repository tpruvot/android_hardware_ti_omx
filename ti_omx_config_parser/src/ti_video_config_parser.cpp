...
OSCL_EXPORT_REF int16 ti_video_config_parser(tiVideoConfigParserInputs *aInputs, tiVideoConfigParserOutputs *aOutputs, char* pComponentName)
{
    ...
    if (aInputs->iMimeType == PVMF_MIME_M4V) //m4v
    {
        ...
        {
            if (width*height > WVGA_MAX_WIDTH*WVGA_MAX_HEIGHT) return -1;
        }
        if ( 0 == oscl_strncmp (pComponentName, ITTIAM_VID_DEC, oscl_strlen (ITTIAM_VID_DEC)) )
        {
            if (width*height > MAX_WIDTH_720P*MAX_HEIGHT_720P) return -1;
        }
    ...
    else if (aInputs->iMimeType == PVMF_MIME_WMV) //wmv
    {
        ...
        if (w*dwdat > WVGA_MAX_WIDTH*WVGA_MAX_HEIGHT)
            return -1
        ...
        if ( // NewCompression != FOURCC_WMV2 && // Blocking WMV7 & WMV8
